...
public class CameraRenderer extends Thread implements SurfaceTexture.OnFrameAvailableListener {
    ...
    private EGLTest mEglCore;


    private android.opengl.EGLSurface mWindowSurface;
     ...
     */
    private String mOutputFile = "/sdcard/test.mp4";
    ...
    private void loadFromShadersFromAssets(String pathToFragment, String pathToVertex) {
        fragmentShaderCode = "#extension GL_OES_EGL_image_external : require\n" +
                ...
                "}";
        vertexShaderCode = "attribute vec2 vPosition;\n" +
     ...
     */
    public void initGL() {
        mEglCore = new EGLTest();

        //create preview surface
        mWindowSurface = mEglCore.createWindowSurface(mSurfaceTexture);
        mEglCore.makeCurrent(mWindowSurface);

        //create recording surface
        mRecordSurface = mEglCore.createWindowSurface(mMediaRecorder.getSurface());
    ...
    public void onFrameAvailable(SurfaceTexture surfaceTexture) {
        ...
        synchronized (this) {
            ...
            if (mIsRecording) {
                ...
                mEglCore.makeCurrent(mRecordSurface);

                setViewport(640, 480);
                draw();

                mEglCore.setPresentationTime(mRecordSurface, surfaceTexture.getTimestamp());
                mEglCore.swapBuffers(mRecordSurface);

                //setViewport(mWindowSurface.getWidth(), mWindowSurface.getHeight());
            }

            mEglCore.makeCurrent(mWindowSurface);
            swapResult = mEglCore.swapBuffers(mWindowSurface);
