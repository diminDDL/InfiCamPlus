...
/*   This thread will attach to the JVM, then call a Java callback.
 *   The reason we need this is because the callback from libuvc
 *   doesn't allow us to do something at the end of the thread that calls.
 *   This will be called by the UVC callback.
 *   Weed to detach the thread from the JVM when we are done.
 */
