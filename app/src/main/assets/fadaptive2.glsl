...
float W(vec3 c[16], int o) {
	float mu = 1.0; /* Should be a positive value close to and lower than one. */
	float n = 8.0; /* Should be positive value. */
...
void main() {
	...
	vec3 c[16];
	c[0] = texture2D(sTexture, (p + vec2(-1.5, -1.5)) * ts).rgb;
	c[1] = texture2D(sTexture, (p + vec2(-0.5, -1.5)) * ts).rgb;
	c[2] = texture2D(sTexture, (p + vec2(+0.5, -1.5)) * ts).rgb;
	c[3] = texture2D(sTexture, (p + vec2(+1.5, -1.5)) * ts).rgb;
	c[4] = texture2D(sTexture, (p + vec2(-1.5, -0.5)) * ts).rgb;
	c[5] = texture2D(sTexture, (p + vec2(-0.5, -0.5)) * ts).rgb;
	c[6] = texture2D(sTexture, (p + vec2(+0.5, -0.5)) * ts).rgb;
	c[7] = texture2D(sTexture, (p + vec2(+1.5, -0.5)) * ts).rgb;
	c[8] = texture2D(sTexture, (p + vec2(-1.5, +0.5)) * ts).rgb;
	c[9] = texture2D(sTexture, (p + vec2(-0.5, +0.5)) * ts).rgb;
	c[10] = texture2D(sTexture, (p + vec2(+0.5, +0.5)) * ts).rgb;
	c[11] = texture2D(sTexture, (p + vec2(+1.5, +0.5)) * ts).rgb;
	c[12] = texture2D(sTexture, (p + vec2(-1.5, +1.5)) * ts).rgb;
	c[13] = texture2D(sTexture, (p + vec2(-0.5, +1.5)) * ts).rgb;
	c[14] = texture2D(sTexture, (p + vec2(+0.5, +1.5)) * ts).rgb;
	c[15] = texture2D(sTexture, (p + vec2(+1.5, +1.5)) * ts).rgb;
