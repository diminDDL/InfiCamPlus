...
public class BaseActivity extends AppCompatActivity {
    ...
    private final ArrayList<PermissionCallback> permissionCallbacks =
        new ArrayList<>();
    private boolean fullscreen =
        false; /* to change the default, look at Settings class. */
    private boolean hideNav = false;
    private static final long hideDelay = 2500;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dv.setOnSystemUiVisibilityChangeListener(i -> {
            if ((i & View.SYSTEM_UI_FLAG_FULLSCREEN) != 0) return;
            deferHide();
        });
        WindowManager.LayoutParams par = win.getAttributes();
        par.rotationAnimation =
    ...
    private final Runnable hideUI = () -> {
        ...
        int uiOptions = 0;
        if (fullscreen) uiOptions |=
            View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_IMMERSIVE;
        if (hideNav) uiOptions |=
    ...
    public void setFullscreen(boolean value) {
        ...
        if (!value) {
            View dv = getWindow().getDecorView();
            int uiOptions =
                dv.getSystemUiVisibility() & ~View.SYSTEM_UI_FLAG_FULLSCREEN;
            dv.setSystemUiVisibility(uiOptions);
        }
    }
