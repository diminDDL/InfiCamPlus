...
public class SurfaceRecorder implements Runnable {
    ...
    private Surface _start(Context ctx, int w, int h, boolean sound)
        throws IOException {
        this.ctx = ctx;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            String fname =
                "vid_" + timeStamp + MUX_EXT; /* MediaStore won't overwrite. */
        ...
        muxerStarted =
            false; /* We start it later, when the codecs report they're configured. */
    ...
    public Surface start(Context ctx, int w, int h, boolean sound)
        throws IOException {
    ...
    public void stop() {
        ...
        fileDescriptor = null;
        if (fileUri != null) Util.scanMedia(ctx, fileUri);
        fileUri = null;
    }
    ...
    public void run() {
        ...
        while (!vidDone || (!sndDone && audioEncoder != null)) {
            ...
            if (encoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                ...
                ByteBuffer encodedData = videoEncoder.getOutputBuffer(
                    encoderStatus
                );
                ...
                if (bufferInfo.size != 0) muxer.writeSampleData(
                    videoTrack,
                    encodedData,
                    bufferInfo
                );
            ...
            if (audioEncoder != null) {
                ...
                if (encoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    if (!muxerStarted) continue;
                    ByteBuffer encodedData = audioEncoder.getOutputBuffer(
                        encoderStatus
                    );
                    assert(encodedData != null);
                    if (
