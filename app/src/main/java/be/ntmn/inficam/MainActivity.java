...
public class MainActivity extends BaseActivity {
    ...
    public final Object frameLock = new Object(); //settingsTherm needs it


    private int picWidth = 1024,
        picHeight = 768;
    private int vidWidth = 1024,
        vidHeight = 768;
    ...
    private final Rect rect =
        new Rect(); /* To use during frames, to avoid allocating it there. */
    ...
    private final SurfaceHolder.Callback surfaceHolderCallback =
        new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(@NonNull SurfaceHolder surfaceHolder) {
                outScreen = new SurfaceMuxer.OutputSurface(
                    surfaceMuxer,
     ...
     */
    private final InfiCam.FrameCallback frameCallback =
        new InfiCam.FrameCallback() {
    ...
    private void getRect(Rect r, int w, int h) {
        /* Git rekt! */
        int sw = w,
            sh = h,
            iw = infiCam.getWidth(),
            ih = infiCam.getHeight();
        if (w == 0 || ih == 0) {
            iw = 4;
            ih = 3;
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        overlayScreen = new Overlay(
            this,
            new SurfaceMuxer.InputSurface(surfaceMuxer)
        );
        overlayRecord = new Overlay(
            this,
            new SurfaceMuxer.InputSurface(surfaceMuxer)
        );
        overlayPicture = new Overlay(
            this,
            new SurfaceMuxer.InputSurface(surfaceMuxer)
        );
        ...
        final ScaleGestureDetector.OnScaleGestureListener scaleListener =
            new ScaleGestureDetector.OnScaleGestureListener() {
                ...
                public boolean onScale(
                    ...
                    messageView.shortMessage(
                        getString(R.string.msg_zoom, (int) (scale * 100.0f))
                    );
                    zl.setText(
                        getString(R.string.zoomlevel, (int) (scale * 100.0f))
                    );
                    return false;
                }
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
                    askPermission(
                        ...
                        granted -> {
                            if (!granted) {
                                messageView.showMessage(
                                    R.string.msg_permdenied_storage
                                );
                                return;
                            }
                            takePic = true;
        ...
        buttonPalette.setOnClickListener(view -> {
            settingsPalette.getPalette().setTo(
                (settingsPalette.getPalette().get() + 1) %
                    settingsPalette.getPalette().getItems().length
            );
            messageView.showMessage(
                Palette.palettes[settingsPalette.getPalette().get()].name
            );
        });
        ...
        buttonLock.setOnClickListener(view -> {
            synchronized (frameLock) {
                if (
                    isNaN(overlayData.rangeMin) && isNaN(overlayData.rangeMax)
                ) {
                    //range is not set
                    ...
                    if (overlayData.rangeMin < start) start = (float) floor(
                        overlayData.rangeMin
                    );
                    if (overlayData.rangeMax > end) end = (float) ceil(
                        overlayData.rangeMax
                    );
                    if (
                        isNaN(overlayData.rangeMin) ||
                        isInfinite(overlayData.rangeMin)
                    ) overlayData.rangeMin = start;
                    if (
                        isNaN(overlayData.rangeMax) ||
                        isInfinite(overlayData.rangeMax)
        ...
        rangeSlider.addOnChangeListener((slider, value, fromUser) -> {
            ...
            if (v.size() < 2 || !fromUser) return;
            if (value == slider.getValues().get(0)) overlayData.rangeMin =
                v.get(0);
        ...
        dialogBackground.setOnClickListener(view ->
        ...
        ImageButton buttonSettingsTherm = findViewById(
            R.id.buttonSettingsTherm
        );
        buttonSettingsTherm.setOnClickListener(view ->
            showSettings(settingsTherm)
        );

        ImageButton buttonSettingsMeasure = findViewById(
            R.id.buttonSettingsMeasure
        );
        buttonSettingsMeasure.setOnClickListener(view ->
        ...
        buttonsLeftLayout =
            (ConstraintLayout.LayoutParams) buttonsLeft.getLayoutParams();
        buttonsRightLayout =
            (ConstraintLayout.LayoutParams) buttonsRight.getLayoutParams();
    }
    ...
    protected void onStart() {
        ...
        IntentFilter batIFilter = new IntentFilter(
    ...
    public void onBackPressed() {
        if (
            dialogBackground.getVisibility() == View.VISIBLE
        ) dialogBackground.setVisibility(View.GONE);
    ...
    private void updateOrientation() {
        ...
        if (
            ...
            buttonsLeftLayout.bottomToBottom =
            ...
            buttonsRightLayout.bottomToBottom =
                ConstraintLayout.LayoutParams.UNSET;
            buttonsRightLayout.leftToLeft = R.id.mainLayout;
            buttonsRightLayout.rightToRight = R.id.mainLayout;
    ...
    private void showSettings(Settings settings) {
        ...
        for (int i = 0; i < dialogs.getChildCount(); ++i) dialogs
            .getChildAt(i)
    ...
    private void startRecording(boolean recordAudio) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            askPermission(
                Manifest.permission.WRITE_EXTERNAL_STORAGE,
                granted -> {
                    if (!granted) messageView.showMessage(
                        R.string.msg_permdenied_storage
                    );
    ...
    public void setImgType(int i) {
        if (imgCompressThread != null) imgCompressThread.lock.lock();
        imgType = i;
        if (imgCompressThread != null) imgCompressThread.lock.unlock();
    }
