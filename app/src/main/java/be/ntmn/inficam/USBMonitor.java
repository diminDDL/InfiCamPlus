...
public abstract class USBMonitor extends BroadcastReceiver {
    ...
    private static final String ACTION_USB_PERMISSION =
        "be.ntmn.inficam.USB_PERMISSION";
    ...
    private final HashMap<UsbDevice, ConnectCallback> callbacks =
        new HashMap<>();

    public void start(Context ctx) {
        /* Recommended use is in onCreate()/onStart(). */
        this.ctx = ctx;
        if (!registered) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                ctx.registerReceiver(this, filter); //TODO: Why is AndroidStudio mad about this line ?
            }
    ...
    public void stop() {
        /* Call this in onDestroy()/onStop(), matching start() call. */
        try {
            registered = false;
            ctx.unregisterReceiver(
                this
            ); /* Prevent resurrection of dead Activities. */
        } catch (Exception e) {
    ...
    public void scan() {
        /* To discover devices already connected on start, call this. */
        if (manager == null) return;
        HashMap<String, UsbDevice> deviceList = manager.getDeviceList();
        for (UsbDevice dev : deviceList.values()) onDeviceFound(dev);
    }

    private void _connect(UsbDevice dev, ConnectCallback cb) {
        UsbDeviceConnection conn = manager.openDevice(dev);
        if (conn != null) cb.onConnected(dev, conn);
    ...
    public void connect(UsbDevice dev, ConnectCallback cb) {
        if (!manager.hasPermission(dev)) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) flags |=
