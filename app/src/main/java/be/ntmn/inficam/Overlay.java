...
public class Overlay {
    ...
    public static class Data {
        ...
        public float rangeMin = NaN,
            rangeMax = NaN;
        public boolean rotate = false,
            mirror = false,
            rotate90 = false; /* Set by Settings. */
    ...
    private final Rect vRect = new Rect(),
        rectTgt = new Rect(); /* Do not alloc each frame! */

    /* These sizes are in fractions of the total width of the bitmap drawn. */
    private static final float smarker = 0.015f; /* Marker size. */
    private static final float wmarker = 0.003f; /* How fat the markers are. */
    private static final float toff =
        0.03f; /* How far to put the text away from marker. */
    private static final float tclearance =
        0.005f; /* How far the text should stay from edges. */
    private static final float textsize = 0.035f;
    private static final float woutline = 0.008f; /* Text outline thickness. */
    private static final float pwidth = 0.038f; /* Palette preview width. */
    private static final float pclearance = 0.016f;
    ...
    public Overlay(Context ctx, SurfaceMuxer.InputSurface is) {
        ...
        lock = AppCompatResources.getDrawable(
            ctx,
            R.drawable.ic_baseline_lock_24_2
        );
    }

    public static MinMaxAvgCet computeMmacRect(
        float[] temp,
        int left,
        int top,
        int right,
    ...
    private void drawText(
        ...
        paintTextOutline.setTextAlign(
            la ? Paint.Align.LEFT : Paint.Align.RIGHT
        );
        cvs.drawText(
            sb,
            0,
    ...
    public void draw(Data d, SettingsPalette settingsPalette, Rect rect) {
        ...
        if (d.showCenter) {
            // TODO this is off by a pixel and we should check the other points too
        ...
        if (d.showPalette) {
            ...
            if (width <= vRect.width()) {
                ...
                Util.formatTemp(
                    sb,
                    Float.isNaN(d.rangeMax) ? d.mmac.max : d.rangeMax,
                    d.tempUnit
                );
                drawText(
                    cvs,
                ...
                if (!Float.isNaN(d.rangeMax)) {
                    int off = (int) paintTextOutline.measureText(
                        sb,
                        0,
                        sb.length()
                    );
                    lock.setBounds(
                        vRect.right - clear - off - isize,
                        vRect.top + iclear,
                        vRect.right - clear - off,
                        vRect.top + iclear + isize
                    );
                    lock.draw(cvs);
                }
                Util.formatTemp(
                    sb,
                    Float.isNaN(d.rangeMin) ? d.mmac.min : d.rangeMin,
                    d.tempUnit
                );
                drawText(
                    cvs,
    ...
    private void drawTPoint(Canvas cvs, Data d, int tx, int ty, float temp) {
        ...
        float xm =
            (((tx + 0.5f) * vRect.width()) /
                (d.rotate90 ? d.fi.height : d.fi.width)) * d.scale;
        if (d.rotate) xm = vRect.width() * d.scale - xm;
        if (d.mirror) xm = vRect.width() * d.scale - xm;
        xm += vRect.left;
        xm -= (vRect.width() * d.scale - vRect.width()) / 2.0f;

        float ym =
            (((ty + 0.5f) * vRect.height()) /
                (d.rotate90 ? d.fi.width : d.fi.height)) * d.scale;
        if (d.rotate) ym = vRect.height() * d.scale - ym;
        ym += vRect.top;
        ...
        boolean la = true;
        if (
            paintTextOutline.measureText(sb, 0, sb.length()) + offX + tclear >
            vRect.right - xm
        ) {
            offX = -offX;
            la = false;
        ...
        offY -= max(
            ym + offY + paintTextOutline.descent() + tclear - vRect.bottom,
            0
        );
        offY -= min(
            ym + offY + paintTextOutline.ascent() - tclear - vRect.top,
            0
        );
